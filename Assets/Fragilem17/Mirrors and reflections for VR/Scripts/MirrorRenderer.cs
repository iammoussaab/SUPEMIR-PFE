...
namespace Fragilem17.MirrorsAndPortals
{
    ...
    public class MirrorRenderer : MonoBehaviour
    {
        ...
        private static bool _warnedAboutUseSubmitRenderRequest = false;
#endif
        ...
        {
#if UNITY_2022_3_OR_NEWER
			if (UseSubmitRenderRequest && !_warnedAboutUseSubmitRenderRequest)
			{
                _warnedAboutUseSubmitRenderRequest = true;
                Debug.LogWarning(Colorize("[MIRRORS] ", DebugColors.Warn, true) + "The Mirror texture will be created using 'SubmitRenderRequest' instead of 'RenderSingleCamera', this allows UI to be correctly reflected at the expense of a runtime error that complainins about recursive rendering not being supported, this runtime error can be safely ignored.", this);
            }
#endif
        }
        ...
        private void UpdateCamera(UnityEngine.Rendering.ScriptableRenderContext src, Camera renderCamera)
        {
            ...
            /*
#if UNITY_EDITOR
            // only update this constantly in the editor and when not in playmode, so things stay reactive while editing
            // when creating the reflection camera this is also done.
            if (!Application.isPlaying) { 
                ...
                _reflectionCamera.GetComponent<Skybox>().material = mySkyboxMaterial;
            }
#endif
*/
        ...
        private void RenderMirrorCamera(UnityEngine.Rendering.ScriptableRenderContext src, Camera reflectionCamera, List<CameraMatrices> cameraMatricesInOrder, Camera.StereoscopicEye eye)
        {
            ...
            for (int i = 0; i < cameraMatricesInOrder.Count; i++)
            {
                ...
                if (matrices.depth >= recursions + 1)
                {
                    ...
                    matrices.mirrorSurface.UpdateMaterial(eye, null, this, matrices.depth, Mathf.Infinity, reflectionCamera.transform.position);
                }
                else
                {
                    ...
                    matrices.mirrorSurface.UpdateMaterial(eye, _ptex.texture, this, matrices.depth, matrices.distance, reflectionCamera.transform.position);
...
#pragma warning restore CS0618 // Type or member is obsolete
                    ...
                    if (li.Count > 0)
                    {
                        foreach (CameraMatrices cm in li)
                        {
                            if (cm != matrices)
                            {
                                ...
                                if (p != null)
                                {
                                    cm.mirrorSurface.UpdateMaterial(eye, p.texture, this, cm.depth, cm.distance, reflectionCamera.transform.position);
                                }
                            }
                        }
                    }
        ...
        {
            PooledTexture tex = _pooledTextures.Find(tex2 => !tex2.fullLock && !tex2.liteLock && tex2.eye == eye);
            if (tex == null)
            {
